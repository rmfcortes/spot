...
export class HomePage implements OnInit, OnDestroy {
  ...
  listenCambios() {
    this.negocioService.listenCambios().query.ref.on('child_changed', data => {
      this.ngZone.run(() => {
        ...
        if (this.negociosPopulares.length > 0) {
          const i = this.negociosPopulares.findIndex(n => n.idNegocio = negocio.idNegocio);
          if (i >= 0) {
            this.negociosPopulares[i].abierto = negocio.abierto;
          }
        }
        if (this.negMatch.length > 0) {
          const i = this.negMatch.findIndex(n => n.idNegocio = negocio.idNegocio);
          if (i >= 0) {
            this.negMatch[i].abierto = negocio.abierto;
          }
        }
      });
    });
