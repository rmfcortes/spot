      ...
      this.ngZone.run(async () => {
        if (status === 'OK') {
          if (results[0]) {
            ...
            let dentro;
            if (this.hasRegion) dentro = await google.maps.geometry.poly.containsLocation(place.geometry.location, this.polygon)
            else {
              for (const region of this.regiones) {
                console.log(region);
                this.polygon = await new google.maps.Polygon({paths: region.ubicacion})
                dentro = await google.maps.geometry.poly.containsLocation(place.geometry.location, this.polygon)
                if (dentro) {
                  this.region = region
                  break
                }
