...
export class NegocioPage {
  ...
  async agregaProductos(prod: Producto[], pasillo, event?) {
    return new Promise(async (resolve, reject) => {
      ...
      if (!this.gestureEnable) {
        ...
        const header = document.getElementById('contenido');
        console.log(header);
        this.animationService.headerGesture(header)
      }
    });
  }
  ...
  addProduct(producto: Producto, id) {
    if (!this.uid) {
      this.uid = this.uidService.getUid();
      if (!this.uid) {
    ...
    if (this.vista === 'lista') {
      ...
      this.plusProduct(producto, id);
    }
    return;
  }

  minusProduct(producto: Producto, id) {
    ...
    if (producto.complementos && producto.complementos.length > 0) {
      producto.complementos.forEach(l => {
        l.complementos.forEach(c => {
  ...
  plusProduct(producto: Producto, id) {
    const ele = document.getElementById(id);
    this.animationService.animBrinca(ele)
    producto.cantidad++;
    this.cuenta += producto.precio;
