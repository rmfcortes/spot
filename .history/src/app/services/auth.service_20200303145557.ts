...
export class AuthService {
      ...
      this.fb.login(['public_profile', 'email'])
        .then(res => {
          ...
          this.authFirebase.auth.signInWithCredential(credential)
            .then(async (response) => {
              await this.setUser(response.user.uid, response.user.displayName);
  ...
  async loginWithEmail(email, pass, nombre) {
    return new Promise(async (resolve, reject) => {
    try {
        ...
        this.setUser(resp.user.uid, nombre);
        this.alertService.presentToast('Bienvenido ' + nombre);
        resolve();
      } catch (error) {
        reject(error);
      }
