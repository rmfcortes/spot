...
export class CategoriasService {
  ...
  getPopulares(): Promise<InfoGral[]> {
    return new Promise((resolve, reject) => {
      const popSub = this.db.list(`functions/`, data => data.orderByChild('visitas').limitToLast(1)).valueChanges()
  ...
  getVisitasNegocios(uid: string): Promise<visistasNegocio[]> {
    return new Promise((resolve, reject) => {
      const visSub = this.db.list(`usuarios/${uid}/visitasNegocio`, data => data.orderByChild('visitas').limitToLast(6)).valueChanges()
