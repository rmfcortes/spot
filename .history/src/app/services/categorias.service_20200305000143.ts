...
export class CategoriasService {
  ...
  getPopulares(): Promise<InfoGral[]> {
    return new Promise((resolve, reject) => {
      const region = this.uidService.getRegion();
      const popSub = this.db.list(`functions/${region}`, data => data.orderByChild('visitas').limitToLast(15)).valueChanges()
  ...
  getMasVendidos(): Promise<MasVendido[]> {
    return new Promise((resolve, reject) => {
      const region = this.uidService.getRegion();
      const popSub = this.db.list(`vendidos/${region}`, data => data.orderByChild('ventas').limitToLast(20)).valueChanges()
