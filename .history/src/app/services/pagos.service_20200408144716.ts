...
export class PagosService {
  ...
  cobrar(pedido: Pedido): Promise<boolean> {
    return new Promise((resolve, reject) => {
      if (this.platform.is ('cordova')) {
        this.http.post('https://us-central1-revistaojo-9a8d3.cloudfunctions.net/request', pedido, {Authorization: 'secret-key-test'})
        ...
        const httpOptions = {
         ...
         this.httpAngular.post('https://us-central1-revistaojo-9a8d3.cloudfunctions.net/request', pedido, httpOptions)
         .subscribe(
          res => {
            console.log(res);
            resolve(true)
