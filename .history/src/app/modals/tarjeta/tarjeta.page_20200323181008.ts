...
export class TarjetaPage implements OnInit {
  ...
  borraVigencia() {
    if (this.tarjeta.expiracion.length === 4) this.tarjeta.expiracion = this.tarjeta.expiracion.substring(0, this.tarjeta.expiracion.length - 1)
  }

  vigenciaCambio() {
    if (this.tarjeta.expiracion.length === 3) this.tarjeta.expiracion = this.tarjeta.expiracion + '/'
  }
