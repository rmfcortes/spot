...
masVendidos: MasVendido[] = [];
  ...
  getVisitas() {
    this.categoriaService.getVisitas(this.uid).then(visitas => {
      ...
      this.categoriasSorted = true;
      console.log(this.categoriasSorted);
    });
  }
  ...
  getNegociosVisitados() {
    ...
    this.categoriaService.getVisitasNegocios(this.uid).then(n => {
      if (n.length > 0) {
        n.forEach(async (x) => {
          this.ofertaService.getStatus(x.idNegocio).then((info: any) => {
