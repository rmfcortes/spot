...
matchFound = false;
  ...
  async buscar() {
    ...
    console.log(this.negociosBusqueda);
    this.negMatch = [];
    const busqueda = this.busqueda.toLocaleLowerCase().split(' ');
    ...
    this.negociosBusqueda.forEach(n => {
      const palabras = n.palabras.toLocaleLowerCase().split(' ');
      if (palabras.length > 0) {
        palabras.forEach(p => {
          busqueda.forEach(b => {
            const s = p.localeCompare(b);
            console.log(s);
            if (s === 0) {
              this.negMatch.push(n);
