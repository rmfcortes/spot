...
export class HomePage implements OnInit, OnDestroy {
      ...
      if (busquedaArray.length > 0) {
        ...
        if (incluir) {
          ...
          setTimeout(async () => {      
            ...
            this.animationService.animBrincaCustom(el, 'scale(.7)', 'scale(1.2)')
  ...
  resetBusqueda() {
    ...
    this.negMatch.forEach(n => {
      const el = document.getElementById(n.idNegocio);
      this.animationService.salida(el)
    })
    setTimeout(() => {      
      this.negMatch = [];
